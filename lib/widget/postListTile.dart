...
Widget postListTile({
  ...
  return GestureDetector(
    ...
    onTap: () {
      print(post.toMap());
      if (post.joiners!.any(
          (element) => element.memberId.toString() == _userController.uid)) {
        if (post.joiners!.any((element) =>
            element.memberId.toString() == _userController.uid &&
            element.owner == true)) {
          snackBar(context: context, title: '방장인 모집입니다');
        } else {
          snackBar(context: context, title: '이미 입장한 모집입니다');
        }
      } else {
        showDialog(
          context: context,
